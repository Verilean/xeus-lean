...
def waveformJSHtml (sessionId : String) (laneNames : List String)
  ...
  String.intercalate "\n" [
    ...
    "  // --- Query throttling ----------------------------------------------",
    "  //",
    "  // Pan / zoom emits a fresh `requestAnimationFrame` every browser",
    "  // tick (~16 ms = 60 fps).  Each frame computes a slightly-different",
    "  // viewport, which means a slightly-different cache key, which means",
    "  // a fresh `comm_msg` to the kernel.  At 60 fps the kernel side",
    "  // queues a result per frame; if the user then switches JupyterLab",
    "  // tabs the iopub channel goes silent, the kernel's ZMQ socket hits",
    "  // its high-water-mark on the publish side, and the comm thread",
    "  // blocks.  When the user switches back, the kernel is wedged.",
    "  //",
    "  // Two cheap mitigations, both pure JS, no kernel changes:",
    "  //",
    "  //   (a) Coalesce repeated queries to ~10 Hz (one every 100 ms).",
    "  //       The user can't perceive smoother pan than that anyway,",
    "  //       and the kernel only has to chew through ~10× fewer",
    "  //       `op:'query'` messages during a drag.",
    "  //   (b) Skip new queries entirely while the document is hidden",
    "  //       (`document.visibilityState !== 'visible'`).  When the",
    "  //       user switches back we fire a single catch-up query for",
    "  //       the *current* viewport, so the canvas redraws fresh data",
    "  //       without queueing 600 stale results.",
    "  let lastReqAt = 0;",
    "  const REQ_MIN_GAP_MS = 100;",
    "",
    ...
    "    if (!ws || ws.readyState !== 1 || !commId) return;",
    "    // (b) hidden tab: don't enqueue new work.  We'll catch up on",
    "    //     visibilitychange.  pending stays untouched so the next",
    "    //     visible-tab call retries the same key.",
    "    if (typeof document !== 'undefined' && document.visibilityState",
    "        && document.visibilityState !== 'visible') return;",
    "    // (a) rate-limit drag-induced queries.  A genuine viewport",
    "    //     change still gets through after the gap; the cache",
    "    //     prevents redundant ones.",
    "    const now = (typeof performance !== 'undefined' && performance.now)",
    "                  ? performance.now() : Date.now();",
    "    if (now - lastReqAt < REQ_MIN_GAP_MS) return;",
    "    lastReqAt = now;",
    ...
    "  }",
    "",
    "  // When the tab becomes visible again, fire a single redraw +",
    "  // catch-up query for the current viewport.  We bypass the",
    "  // throttle here because this is a one-shot event, not a drag.",
